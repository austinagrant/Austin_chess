...
class W_P(Piece):
    ...
    def __init__(self, board=0b1111111100000000):
        super(W_P, self).__init__(board, True, 100)
    ...
    def alt_move_rule_2(self, forward1:int, occ:int) -> int:
        return 0xff00 & forward1 & ~(occ >> 16)
    def alt_attack_rule_1(self, opp_board:int) -> int:
        return  self.bitboard & 0x7f7f7f7f7f7f7f7f & (opp_board >> 9)
    def alt_attack_rule_2(self, opp_board:int) -> int:
        return self.bitboard & 0xfefefefefefefefe & (opp_board >> 7)
    ...
    def generateMoves(self, team_board=0, opp_board=0) -> Generator['Move', None, None]:
        ...
        forward_copy = int(forward1)
        while forward1 != 0 :
            msb = self.getMSB(forward1)
            m = Move(self, msb, msb + 8)
            m.pawn_transform = msb >= 48
            forward1 = forward1 ^ (1 << msb)
            # print("push1", m)
            yield m
        forward2 = self.alt_move_rule_2(forward_copy, occ)
        while forward2 != 0:
        ...
        attackLeft = self.alt_attack_rule_1(opp_board)
        while attackLeft != 0:
        ...
        attackRight = self.alt_attack_rule_2(opp_board)
        while(attackRight != 0):
